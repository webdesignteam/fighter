...
WHERE
	...
	{		
		...
		$SQL_QUERY ="SELECT * FROM `tutorials` JOIN `tutorial_series` ON `tutorial_series`.`SERIES_ID` = `tutorials`.`TUTORIAL_SERIES_ID` WHERE `tutorials`.`DIVISION` = '$DIVISION_ID' AND `tutorials`.`TUTORIAL_STATUS` = '1' && '$today' BETWEEN tutorial_series.SERIES_START_DATE AND tutorial_series.SERIES_END_DATE";
